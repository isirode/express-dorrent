...
if (startPod == "true") {
    ...
    exec.with {
        ...
        run "podman run -v " + volumeSource + ":/app/public -p 5100:5100 --detach --rm --name " + containerName + " " + remoteImageName
...
if (pushNginxConf) {
    var session = getSSHSession(username, password, host, port)
    session.setTimeout(30000);
    var sftp = new Sftp(session)
    sftp.with {
        put nginxLocalFileName, nginxRemoteFileName
    }
}
